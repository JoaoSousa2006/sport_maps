...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sqlUpdate = "UPDATE tblplaces SET NamePlace = ?, AdressPlace = ?, EmailPlace = ?, PhonePlace = ?, PricePlace = ?, SportType = ?, LatPlace = ?, LongPlace = ? WHERE idPlace = ?";
    $stmtUpdate = $connection->prepare($sqlUpdate);
    
    // 'ssssssddi' para (NamePlace, AdressPlace, EmailPlace, PhonePlace, PricePlace, SportType, LatPlace, LongPlace, idPlace)
    $stmtUpdate->bind_param("ssssssddi", $newNamePlace, $newAdressPlace, $newEmailPlace, $newPhonePlace, $newPricePlace, $newSportType, $newLatPlace, $newLongPlace, $idPlace);
    ...
    <div class="content">
      ...
      <div class="form__content">
        ...
        <form action="edit_local.php" method="POST">
          ...
          <buttonn type="submit" class="register m_in">Salvar Alterações</button>
        </form>
        <style>
          .m_in {
            margin-inline: 10vw;
